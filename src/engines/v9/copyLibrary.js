...
function heroCopy(profile, sourceTitle = "") {
  const { name, isOmega3, isBerberine, isRedGinseng, story } = profile;
  if (story) return { title: story.hero(name), body: story.heroBody };
  if (isOmega3) return { title: "캡슐보다 먼저 봐야 할 숫자가 있습니다", body: `${name}${particle(name, "은", "는")} EPA·DHA 함량과 원료, 품질 관리 기준을 함께 살펴보는 것이 중요합니다.` };
  if (isBerberine) return { title: "원료가 다르면 선택도 달라집니다", body: `${name}${particle(name, "은", "는")} 원료의 출처와 배합 구성, 품질 정보를 함께 살펴볼 때 제품의 기준이 더 분명해집니다.` };
...
export function makeCopy(intent, profile, source = {}, index = 0) {
  ...
  if (intent === "closing") {
    if (story) return { title: `${story.label}의 핵심 기준을 확인하고 나에게 맞게 선택하세요`, body: `${story.flowItems.map((v) => v.title).join(", ")}를 비교해 ${name}의 차이를 분명하게 살펴보세요.` };
